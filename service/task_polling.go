...
const defaultTaskPollingQueryLimit = 1000
...
func updateVideoTasks(ctx context.Context, platform constant.TaskPlatform, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	for _, taskId := range taskIds {
		...
		gopool.Go(func() {
			...
			// Adaptors may keep mutable request state, so every concurrent task must
			// receive its own adaptor and RelayInfo instance.
