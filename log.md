...
## 2026-08-25
- **Classic 模型广场快捷编辑完整回显**：修复管理员快捷编辑按钮只能按新建模式预填模型名称、无法载入既有配置的问题。`/api/pricing` 新增可选 `model_meta_id`，指向卡片实际继承的数据库模型元数据来源；Classic 根据该 ID 复用 `EditModelModal` 编辑模式，通过现有 `GET /api/models/:id` 完整回显图标、描述、标签、供应商、端点、状态、参与官方同步和名称匹配方式。未配置模型仍进入创建模式并精确预填卡片模型名；前缀、包含、后缀规则匹配的卡片会编辑实际来源规则而不是误建精确记录。关键路径：`model/pricing.go`、`model/pricing_endpoint_test.go`、`web/classic/src/components/table/model-pricing/layout/PricingPage.jsx`、`web/classic/src/components/table/model-pricing/view/card/PricingCardView.jsx`。
- **验证结果**：`go test ./model -count=1`、`go build ./...`、相关 Classic JSX 定向 `prettier --check`、`web/classic` 下 `bun run build` 与 `git diff --check` 均通过；后端回归测试覆盖精确元数据和前缀规则元数据均返回正确来源 ID。
- **Classic 操练场 Dashboard 鉴权修复**：生产日志确认 `POST /pg/chat/completions` 返回 401 的原因不是缺少用户 `sk-...` Key，而是 Classic 原生 `fetch` / `sse.js` 绕过 Axios，仅发送 `New-Api-User`、未发送 Dashboard Bearer。新增 `getValidAccessToken()`，在请求前检查短期 Access Token 并复用现有单飞 Refresh；流式和非流式请求统一携带 `Authorization: Bearer ...`，无法恢复认证时在本地结束加载并提示重新登录，不再发送必然失败的请求。关键路径：`web/classic/src/helpers/auth-session.js`、`web/classic/src/hooks/playground/useApiRequest.jsx`。
- **Classic Console 认证初始化门禁**：有缓存用户资料时，`PageLayout` 在 Refresh Cookie bootstrap 完成前不挂载 Console 的 Header、Sider 和受保护页面，避免硬刷新或菜单进入时先并发发出 `/api/user/self`、`/api/user/models`、`/api/user/self/groups` 等无 Bearer 请求，再触发 401 Refresh 重试造成阻塞感；公共页面和无缓存用户的登录跳转不受影响。Axios 拦截器同时改为不覆盖调用方显式 `Authorization`。关键路径：`web/classic/src/components/layout/PageLayout.jsx`、`web/classic/src/helpers/api.js`。
- **Classic 钱包初始化误报修复**：`top-up quota limit exceeded` 是上游钱包 int32 容量保护，不是 HTTP 429。保留后端 `ValidateTopUpQuotaCapacity` 和支付/结算安全检查，仅删除钱包读取配置后自动调用 `/api/user/amount` 的行为；现在初次打开钱包只初始化配置和金额状态，用户主动输入金额、选择支付方式或支付时才执行金额预览与容量校验。关键路径：`web/classic/src/components/topup/index.jsx`。
- **Classic 全局 Dashboard 请求主动续期**：生产日志确认 `/api/data/...`、`/api/user/self` 等普通及管理员接口会先 401，再调用 `/api/user/auth/refresh` 并重试，阻塞并非钱包专属。统一 Axios 请求拦截器现在在已登录请求发送前通过 `getValidAccessToken()` 检查短期 Token，临近过期时复用单飞 Refresh，显式 Authorization 与登录请求不受影响；401 重试仅保留为服务端提前撤销或状态失步的异常兜底。关键路径：`web/classic/src/helpers/api.js`。
- **Classic Console 全覆盖认证门禁与原生管理流修复**：直接进入任何 `/console/*` 路由时，无论 localStorage 是否有缓存用户，都等待 Refresh Cookie bootstrap 后再挂载受保护页面；Ollama 模型拉取原生 SSE 也改为请求前获取有效 Dashboard Token，避免绕开 Axios 后使用过期 Bearer。关键路径：`web/classic/src/components/layout/PageLayout.jsx`、`web/classic/src/components/table/channels/modals/OllamaModelModal.jsx`。
- **Classic 操练场/钱包首轮验证结果**：首轮 5 个 Classic JS/JSX 文件定向 `prettier --check` 通过，`web/classic` 下 `bun run build` 通过；定向 ESLint 仍被项目现有 ESLint/AJV 初始化错误 `defaultMeta` / `missingRefs` 阻断；`git diff --check` 通过。
- **Classic 全局 Dashboard 鉴权本轮验证**：本轮 3 个 Classic JS/JSX 文件定向 `prettier --check` 通过，`web/classic` 下 `bun run build` 通过，`git diff --check` 通过。
