...
- **Classic 按秒计费验证**：`web/classic` 下 `bun run build` 通过，构建产物已包含“按秒计费”、`per-second` 与 `per_second`；`git diff --check` 通过。`bun run i18n:sync` 因当前 workspace 缺少 `@inquirer/password/dist/index.js` 无法启动，ESLint 因现有 AJV 依赖错误 `defaultMeta` 无法启动，均不是本次 JSX 构建失败。当前修改仅在本地 `hub` 工作区，尚未提交、推送或生成生产镜像。
