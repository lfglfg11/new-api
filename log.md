...
- **验证结果**：上述 5 个 Classic JS/JSX 文件定向 `prettier --check` 通过，`web/classic` 下 `bun run build` 通过；定向 ESLint 仍被项目现有 ESLint/AJV 初始化错误 `defaultMeta` / `missingRefs` 阻断；`git diff --check` 通过。
