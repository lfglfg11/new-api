...
> Classic 模型广场的卡片视图必须为管理员提供就地配置模型元数据的快捷入口：管理员点击卡片右上角编辑按钮后，直接打开模型管理现有的“创建新的模型”表单，并把当前卡片的完整 `model_name` 预填到模型名称字段。普通用户和未登录访问者不能看到该按钮。

必须保留：

- 前端展示权限使用当前登录用户角色判断，仅 `role >= 10` 渲染快捷编辑按钮。
- 按钮点击必须阻止卡片点击事件，不能同时打开模型详情侧滑页。
- 快捷入口必须复用 `EditModelModal` 的创建模式，不复制第二套模型表单，也不新增独立后端接口。
- 传入 `{ id: undefined, model_name: 当前模型名称 }`，确保表单按“新建”模式打开、模型名称精确预填，并沿用现有精确名称匹配默认值。
- 提交继续调用现有受 `middleware.AdminAuth()` 保护的 `POST /api/models/`；前端隐藏只是体验控制，不能替代后端鉴权。
- 创建成功后刷新 `/api/pricing` 数据，使模型广场及时显示新配置的图标、描述、标签和供应商信息。

#### 当前实现路径

- `web/classic/src/components/table/model-pricing/layout/PricingPage.jsx`
  - 根据 `userState.user.role` 判断管理员身份。
  - 管理快捷创建状态，并复用 `web/classic/src/components/table/models/modals/EditModelModal.jsx`。
- `web/classic/src/components/table/model-pricing/view/card/PricingCardView.jsx`
  - 在卡片右上角渲染管理员专属编辑按钮。
  - 阻止事件冒泡并传递当前卡片的 `model_name`。

#### 上游同步保留策略

- 上游调整模型广场卡片布局时，将快捷按钮迁移到新的卡片操作区，不能因卡片组件重构而静默丢失。
- 上游调整模型管理表单路径或 props 时，继续复用其创建入口；不要复制表单实现，以避免两套字段和校验规则漂移。
- 上游调整用户角色状态结构时，重新确认管理员阈值仍为 `role >= 10`，同时保留后端 `/api/models/` 的管理员鉴权。
- 该能力不需要修改 `/api/pricing` 响应或为模型元数据暴露数据库 ID；模型名称是快捷创建的唯一预填值，以减少后端冲突面。

#### 可重复验收

- [ ] 管理员打开 Classic 模型广场卡片视图时，每张模型卡片右上角显示编辑按钮。
- [ ] 普通用户和未登录访问者打开同一页面时不渲染编辑按钮。
- [ ] 点击编辑按钮只打开“创建新的模型”侧滑表单，不同时打开模型详情。
- [ ] 模型名称字段完整等于所点击卡片的 `model_name`，名称匹配类型默认为精确匹配。
- [ ] 提交成功后侧滑表单关闭，模型广场刷新并显示新增的模型元数据。
- [ ] 直接绕过前端调用 `POST /api/models/` 时，非管理员仍被后端拒绝。
