...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/qrcode", controller.GetPayQRCode)
		//apiRouter.GET("/midjourney", controller.GetMidjourney)
		apiRouter.Any("/notify", controller.AlipayNotify)
		...
		apiRouter.GET("/reset_password", middleware.ScopedCriticalRateLimit("auth-password-reset"), middleware.TurnstileCheck(), controller.SendPasswordResetEmail)
		apiRouter.POST("/user/reset", middleware.ScopedCriticalRateLimit("auth-password-reset"), anonymousRequestBodyLimit, controller.ResetPassword)
		// OAuth routes - specific routes must come before :provider wildcard
		apiRouter.POST("/oauth/state", middleware.ScopedCriticalRateLimit("auth-oauth"), middleware.DisableCache(), middleware.TryUserAuth(), anonymousRequestBodyLimit, controller.GenerateOAuthCode)
		apiRouter.POST("/oauth/email/bind", middleware.UserAuth(), middleware.CriticalRateLimit(), controller.EmailBind)
		// Non-standard OAuth (WeChat, Telegram) - keep original routes
		apiRouter.GET("/oauth/wechat", middleware.ScopedCriticalRateLimit("auth-login"), middleware.DisableCache(), controller.WeChatAuth)
		apiRouter.POST("/oauth/wechat/bind", middleware.UserAuth(), middleware.CriticalRateLimit(), controller.WeChatBind)
		apiRouter.GET("/oauth/telegram/login", middleware.ScopedCriticalRateLimit("auth-login"), middleware.DisableCache(), controller.TelegramLogin)
		...
		apiRouter.GET("/oauth/:provider", middleware.ScopedCriticalRateLimit("auth-oauth"), middleware.DisableCache(), middleware.TryUserAuth(), controller.HandleOAuth)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			userRoute.POST("/auth/logout", middleware.SessionCookieOriginGuard(), middleware.ScopedCriticalRateLimit("auth-logout"), middleware.DisableCache(), controller.AuthLogout)
			userRoute.POST("/register", middleware.ScopedCriticalRateLimit("auth-register"), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.Register)
			userRoute.POST("/login", middleware.ScopedCriticalRateLimit("auth-login"), middleware.DisableCache(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.Login)
			userRoute.POST("/login/2fa", middleware.ScopedCriticalRateLimit("auth-login"), middleware.DisableCache(), anonymousRequestBodyLimit, controller.Verify2FALogin)
			userRoute.POST("/passkey/login/begin", middleware.ScopedCriticalRateLimit("auth-login"), middleware.DisableCache(), anonymousRequestBodyLimit, controller.PasskeyLoginBegin)
			userRoute.POST("/passkey/login/finish", middleware.ScopedCriticalRateLimit("auth-login"), middleware.DisableCache(), anonymousRequestBodyLimit, controller.PasskeyLoginFinish)
			//userRoute.POST("/tokenlog", middleware.CriticalRateLimit(), controller.TokenLog)
			userRoute.POST("/epay/notify", anonymousRequestBodyLimit, controller.EpayNotify)
			userRoute.GET("/epay/notify", controller.EpayNotify)
			userRoute.Any("/alipay/notify", controller.AlipayNotify)
			...
			{
				selfRoute.GET("/qrcode", controller.GetPayQRCode)
