...
// -------------------------------
// 模型定价计算工具函数
export const isPerSecondBilling = (record) => {
  if (record?.billing_mode === 'per_second') return true;
  if (record?.billing_mode === 'per_request') return false;
  return record?.task_billing_unit === 'second';
};

export const matchesPricingQuotaType = (record, filterQuotaType) => {
  if (filterQuotaType === 'all') return true;
  if (filterQuotaType === 'per-second') {
    return record?.quota_type === 1 && isPerSecondBilling(record);
  }
  if (filterQuotaType === 1) {
    return record?.quota_type === 1 && !isPerSecondBilling(record);
  }
  return record?.quota_type === filterQuotaType;
};
...
export const calculateModelPrice = ({
  ...
  if (record.quota_type === 1) {
    ...
    return {
      price: displayVal,
      billingUnit: isPerSecondBilling(record) ? 'second' : 'request',
      isPerToken: false,
      isTokensDisplay: false,
      usedGroup,
      usedGroupRatio,
...
export const getModelPriceItems = (priceData, t, quotaDisplayType = 'USD') => {
  ...
  const fixedUnitLabel = priceData.billingUnit === 'second' ? t('秒') : t('次');
  return [
    {
      key: 'fixed',
      label: t('模型价格'),
      value: priceData.price,
      suffix: ` / ${fixedUnitLabel}`,
    },
  ].filter(
    (item) =>
      item.value !== null && item.value !== undefined && item.value !== '',
